...
| Backend Python runtime root | backend virtualenv | Playbook Python tooling should use the same venv as the backend |
| Frontend toolchain | Vite `6.2.2`, TypeScript `5.8.2`, Vitest `2.1.9`, Playwright `1.58.2` | Keep aligned with Node `24.x` |
| `safrs-jsonapi-client` source rule | default to cloning the latest upstream `https://github.com/thomaxxl/safrs-jsonapi-client` checkout into local `tmp/safrs-jsonapi-client`, then install from `file:../tmp/safrs-jsonapi-client` | Use a local materialized git checkout, not a raw release URL |
