...
| Resource | Exposed through SAFRS | Canonical SAFRS resource path | SAFRS backing lane | Resource class | Default menu presence | List | Show | Create | Edit | Delete | Read-only fields | Derived backend-managed fields | Custom endpoint supplements? | Why ordinary SAFRS is insufficient | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `<resource>` | `<yes/no/internal/singleton/deferred>` | `<resource path or none>` | `<table model/view model/selectable/jsonapi_rpc/exception>` | `<core/reference/status/etc.>` | `<yes/no>` | `<yes/no>` | `<yes/no>` | `<yes/no>` | `<yes/no>` | `<field list or none>` | `<field list or none>` | `<endpoint list or none>` | `<reason or none>` | `<notes>` |
...
- for every DB-backed summary/query surface, whether the preferred lane is a
  view/selectable-backed SAFRS resource, `jsonapi_rpc`, or approved exception
