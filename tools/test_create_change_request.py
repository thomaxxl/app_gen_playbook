...
class CreateChangeRequestTests(unittest.TestCase):
    def test_creates_narrow_change_packet_and_inbox_reads(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            change_root = repo_root / "runs/current/artifacts/product/changes" / change_id
            ...
            self.assertIn(f"runs/current/artifacts/product/changes/{change_id}/request.md", inbox_text)
            self.assertIn(f"runs/current/artifacts/product/changes/{change_id}/affected-artifacts.md", inbox_text)
            self.assertIn(f"runs/current/artifacts/product/changes/{change_id}/affected-app-paths.md", inbox_text)
            self.assertIn(f"runs/current/artifacts/product/changes/{change_id}/reopened-gates.md", inbox_text)
