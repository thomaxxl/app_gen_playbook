...
PASS_VALUES = {"approved", "pass", "passed", "reviewed"}
...
def collect_issues(repo_root: Path) -> list[dict[str, str]]:
    ...
    for section_name, rows, columns in expected_sections:
        if _table_columns(rows) != columns:
            issues.append(
                {
                    "path": review_path,
                    "reason": f"integration review {section_name} must use exact columns {list(columns)}",
                }
            )
    ...
    for story in plan.get("story_reviews", plan.get("stories", [])):
        ...
        if row is None:
            issues.append({"path": review_path, "reason": f"integration review is missing Story Coverage row for {story_id}"})
            continue
        if normalized_text(row.get("Decision", "")) not in PASS_VALUES:
            issues.append({"path": review_path, "reason": f"integration review story {story_id} must use Decision approved/pass/reviewed"})
        if _is_placeholder(row.get("Independent Test Evidence", "")):
            issues.append({"path": review_path, "reason": f"integration review story {story_id} is missing Independent Test Evidence"})
        if _is_placeholder(row.get("Scenario Coverage", "")):
            issues.append({"path": review_path, "reason": f"integration review story {story_id} is missing Scenario Coverage detail"})
        if _is_placeholder(row.get("Notes", "")):
            issues.append({"path": review_path, "reason": f"integration review story {story_id} notes are missing or placeholder"})
        if story.get("ui_surface_required"):
            ...
            if missing_surface_ids:
                issues.append(
                    {
                        "path": review_path,
                        "reason": f"integration review story {story_id} is missing supporting surface IDs {missing_surface_ids}",
                    }
                )
    ...
    for actor in scope.get("required_actor_coverage", []):
        ...
        if row is None:
            issues.append({"path": review_path, "reason": f"integration review Actor Coverage is missing actor {actor}"})
            continue
        if _is_placeholder(row.get("Covered Story IDs", "")) or _is_placeholder(row.get("Evidence Summary", "")):
            issues.append({"path": review_path, "reason": f"integration review actor {actor} has placeholder coverage evidence"})

    for story_type in scope.get("story_type_catalog", []):
        row = type_by_name.get(str(story_type).lower())
        if row is None:
            issues.append({"path": review_path, "reason": f"integration review Story Type Coverage is missing story type {story_type}"})
            continue
        if _is_placeholder(row.get("Covered Story IDs", "")) or _is_placeholder(row.get("Evidence Summary", "")):
            issues.append({"path": review_path, "reason": f"integration review story type {story_type} has placeholder coverage evidence"})

    for scenario_check in scope.get("required_scenario_checks", []):
        row = scenario_by_check.get(str(scenario_check).lower())
        if row is None:
            issues.append({"path": review_path, "reason": f"integration review Scenario Depth Coverage is missing check {scenario_check}"})
            continue
        if _is_placeholder(row.get("Covered Story IDs", "")) or _is_placeholder(row.get("Evidence Summary", "")):
            issues.append({"path": review_path, "reason": f"integration review scenario check {scenario_check} has placeholder evidence"})

    for page_id in scope.get("required_custom_pages", []):
        row = page_by_id.get(page_id)
        if row is None:
            issues.append({"path": review_path, "reason": f"integration review Page Coverage is missing page {page_id}"})
            continue
        if _is_placeholder(row.get("Covered Story IDs", "")) or _is_placeholder(row.get("Evidence Summary", "")):
            issues.append({"path": review_path, "reason": f"integration review page {page_id} has placeholder coverage evidence"})
    ...
    for surface in plan.get("surfaces", []):
        ...
        if row is None:
            issues.append({"path": review_path, "reason": f"integration review Route Coverage is missing route {route_id}"})
            continue
        if normalized_text(row.get("Path", "")) != normalized_text(surface.get("path", "")):
            issues.append(
                {
                    "path": review_path,
                    "reason": f"integration review route {route_id} path drifted from {surface.get('path', '')}",
                }
            )
        if _is_placeholder(row.get("Covered Story IDs", "")) or _is_placeholder(row.get("Evidence Summary", "")):
            issues.append({"path": review_path, "reason": f"integration review route {route_id} has placeholder coverage evidence"})
