...
class CheckCompletionTests(unittest.TestCase):
    ...
    def test_acceptance_reference_fidelity_check_accepts_hyphenated_explicit_wording(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            (repo_root / ".git").mkdir()
            write_file(
            ...
            write_file(
                ...
                "\n".join(
                    [
                        ...
                        "",
                        "## Actor Coverage",
                        "- Requester is covered.",
                        "",
                        "## Story Type Coverage",
                        "- crud is covered.",
                        "",
                        "## Scenario Depth Coverage",
                        "- happy path, alternate path, negative validation, empty state, and permission context are covered.",
                        "",
                        "## Page Coverage",
                        "- PAGE-001 is covered.",
                        "",
    ...
    def test_terminal_delivery_approval_suppresses_stale_coverage_policy_blockers(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            (repo_root / ".git").mkdir()
            write_file(
    ...
    def test_accepts_runtime_failed_ui_preview_manifest_capture_status(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            (repo_root / ".git").mkdir()
            write_file(
    ...
    def test_runtime_failed_preview_manifest_is_allowed_but_still_blocking(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            (repo_root / ".git").mkdir()
            write_file(
            ...
            matching = [blocker for blocker in blockers if blocker["path"] == "runs/current/evidence/ui-previews/manifest.md"]
            self.assertFalse(any(blocker["kind"] == "ui-preview-manifest-unstructured" for blocker in matching))
            self.assertTrue(any(blocker["kind"] == "ui-preview-runtime-failed" for blocker in matching))
