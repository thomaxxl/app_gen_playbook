...
class RunPlaybookWorkerContractTests(unittest.TestCase):
    ...
    def test_runner_schedules_periodic_ceo_progress_audits(self) -> None:
        ...
        self.assertIn("write_ceo_progress_audit_summary()", script)
        self.assertIn('python3 "$ROOT/tools/render_ceo_progress_summary.py"', script)
        ...
        self.assertIn("CEO Progress Executive Summary", (Path(__file__).resolve().parents[1] / "tools" / "render_ceo_progress_summary.py").read_text(encoding="utf-8"))
        ...
        self.assertIn('summary_rel="${summary_path#$ROOT/}"', script)
        self.assertIn('- executive summary: $summary_rel', script)
