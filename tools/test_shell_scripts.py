...
class ShellScriptSyntaxTests(unittest.TestCase):
    ...
    def test_steer_script_pause_writes_direct_orchestrator_request(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pause_request = Path(result.stdout.strip())
            self.assertTrue(pause_request.is_file())
            pause_text = pause_request.read_text(encoding="utf-8")
            self.assertIn("request_mode: drain-pause", pause_text)
            self.assertIn("do not start new work", pause_text)
            self.assertFalse((repo_root / "runs" / "current" / "role-state" / "ceo" / "inbox").exists())
    ...
    def test_steer_script_kill_writes_kill_request_and_signals_runner(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            kill_request = Path(result.stdout.strip())
