...
def agent_failure_message(jsonl_path: Path, result_path: Path) -> str | None:
    ...
    if not result_content:
        return "agent run completed without a final agent message"
...
def codex_failure_message(jsonl_path: Path, result_path: Path) -> str | None:
    return agent_failure_message(jsonl_path, result_path)


def main(argv: list[str], *, prog_name: str = "assert_agent_success.py") -> int:
    if len(argv) != 3:
        print(f"usage: {prog_name} <events.jsonl> <result.md>", file=sys.stderr)
        return 2

    failure_message = agent_failure_message(Path(argv[1]), Path(argv[2]))
    if failure_message:
