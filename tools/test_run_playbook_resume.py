...
class RunPlaybookResumeTests(unittest.TestCase):
    ...
    def test_resume_clears_stale_tagged_execution_prereq_operator_action_when_prereqs_are_now_ready(self) -> None:
        source_repo = Path(__file__).resolve().parents[1]
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse((repo_root / "runs" / "current" / "orchestrator" / "operator-action-required.md").exists())
            archive_dir = repo_root / "runs" / "current" / "evidence" / "orchestrator" / "operator-action-archive"
            archived = list(archive_dir.glob("operator-action-required.execution-prereqs-cleared.*.md"))
            self.assertTrue(archived)
    ...
    def test_resume_clears_kill_requested_and_continues(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (repo_root / "runs" / "current" / "orchestrator" / "kill-requested.md").write_text(
                "# Kill Requested\n\nStop now.\n",
                encoding="utf-8",
            )
